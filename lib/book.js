((window, document, undefined) => {
    ...
    function book() {
        ...
        _self.next = function() {
			if(_self.mode === 'double'){
				_self.load(_self.page_number + 2);
			}else{
				_self.load(_self.page_number + 1);
			}
            _self.direction = 'forward';
        ...
        // This is redundant. In our case the content that is to be shown 
        // inside the pages is loaded separately and thus it is a part of the pageObject
        // array elements.        		
		// _self.load = function(pageNo){
		// 	_self.page_number = pageNo;
		// 	let book = 	document.getElementById('book');
		// 	let containerList = book.getElementsByTagName('div');
		// 	//TODO: Update the style of the page to display the current page.
		// 	for(let i = 0; i < containerList.length; i++){
		// 		containerList[i].textContent = _self.settings.pages[pageNo++];
		// 	}
		// }
