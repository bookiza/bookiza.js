import leafer from '../modules/mode.js'
...
;((w, d, undefined) => {
  function flippy () {
    ...
    _book.mode = leafer.getMatch('all and (min-width: 870px)') ? 'single' : 'double'

    leafer.onChange('all and (min-width: 871px)', match => {
      _book.mode = match ? 'double' : 'single'

      let current_page = _book.settings.start_page
    ...
    function cleanSlate () {
      ...
      _book.length = _book.pages.length // Check if _book.length is even
    ...
    function removeChildren (node) {
      node.innerHTML = ''
    // while (node.hasChildNodes())         // Test performant?
    ...
    function estimatePages (current_page) {
      switch (_book.mode) {
        case 'single':
          ...
          _book.leaf = [parseInt(current_page - 1)]

          console.log('mode', _book.mode, 'LEAF', _book.leaf)

          _book.range = _book.pages.slice(parseInt(_book.leaf[0]) - 2, parseInt(_book.leaf[0]) + 1) // Array indices start at 0.
          break

        case 'double':
          if (isEven(parseInt(current_page))) { _book.leaf = [parseInt(current_page), parseInt(current_page + 1)] }
          if (isOdd(parseInt(current_page))) { _book.leaf = [parseInt(current_page - 1), parseInt(current_page)] }

          console.log('mode', _book.mode, 'LEAF', _book.leaf)

          _book.range = _book.pages.slice(parseInt(_book.leaf[0]) - 3, parseInt(_book.leaf[1]) + 2) // Array indices start at 0.
          break
    ...
    function applyStyles (pageObj, classes) {
      // Add style classes to HTML if required.
      console.log(pageObj, 'HAS', pageObj.classList.contains(classes))
      switch (_book.mode) {
        case 'single':
          if (pageObj.classList.contains('left')) pageObj.classList.remove('left')
