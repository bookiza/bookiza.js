...
;((w, d, undefined) => {
  function flippy () {
    ...
    _book.init = (settings = { duration: 500, animation: true, curl: true, allowZoom: false }) => {
      oneTimeInit(settings)
      return
    }

    leafer.onChange('(orientation: landscape)', match => {
    ...
    function oneTimeInit (settings) {
      ...
      _book.mode = leafer.getMatch('(orientation: landscape)') ? 'double' : 'single'
    ...
    function printPages (node) {
      ...
      node.appendChild(docfrag) // insertBefore?
    }
