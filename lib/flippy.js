...
(((w, d, undefined) => {
    ...
    function _setView(currentPage = 1) {
        switch (_book.mode) {
            case 'portrait':
                ...
                _book.viewablePages = [ _book.pages[`${currentPage }`] ]
                break
            ...
            case 'landscape':
                if (isEven(parseInt(currentPage))) { 
                    ...
                    ***/
                    ...
                    _book.currentView = [`${currentPage}`, `${parseInt(currentPage) + 1}` ] 
                    _book.viewablePages = [ _book.pages[`${currentPage}`], _book.pages[`${parseInt(currentPage) + 1}`] ]
                } else {
                    _book.currentView = [`${parseInt(currentPage) - 1}`, `${currentPage}`] 
                    _book.viewablePages = [ _book.pages[`${parseInt(currentPage) - 1}`], _book.pages[`${currentPage}`] ]
                }
    ...
    function _printViewablePages(node) {
        ...
        const pageList = _book.viewablePages.map((page, index) => {
