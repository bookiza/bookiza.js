...
(((n, w, d) => {
  ...
  const _initializeSuperbook = (node, settings = { speed: 500, animation: true, peel: true, zoom: true }) => {
    ...
    _book.currentView = _setView(_book.currentPage)
  ...
  _viewer.onChange('(orientation: landscape)', match => {
	...
	// Cyclic array
  const _setCurrentPage = startPage => (startPage === undefined) ? 1 : (parseInt(startPage) > 0 && parseInt(startPage) < parseInt(_book.pages.length)) ? parseInt(startPage) % parseInt(_book.pages.length) : (parseInt(startPage) % parseInt(_book.pages.length) === 0) ? parseInt(_book.pages.length) : parseInt(startPage) < 0 ? parseInt(_book.pages.length) + 1 + parseInt(startPage) % parseInt(_book.pages.length) : parseInt(startPage) % parseInt(_book.pages.length)
